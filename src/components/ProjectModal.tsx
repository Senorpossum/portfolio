...
export default function ProjectModal({ project, onClose }: ProjectModalProps) {
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl bg-zinc-950 border border-zinc-800 shadow-2xl p-6 sm:p-8 space-y-6"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-5 right-5 p-1.5 rounded-lg bg-zinc-900 text-zinc-400 hover:text-zinc-100 hover:bg-zinc-800 border border-zinc-800 transition-colors"
        ...
        {/* Header */}
        <div>
          <div className="flex items-center gap-2.5 text-xs font-mono text-zinc-400 mb-2">
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {flow.map((item) => (
              <div
                key={item.step}
                className="p-3.5 rounded-lg bg-zinc-900/40 border border-zinc-800/80 space-y-1"
              >
                <div className="flex items-center gap-2">
                  <span className="text-[10px] font-mono font-semibold px-1.5 py-0.5 rounded bg-zinc-800 text-cyan-400">
                    {item.step}
                  </span>
        ...
        <div className="space-y-4 text-xs leading-relaxed bg-zinc-900/20 p-4 rounded-xl border border-zinc-800/60">
          <div>
            <span className="font-mono text-[11px] text-zinc-400 uppercase tracking-wider block mb-1">
              Why I Built It
            </span>
        ...
        <div className="space-y-2">
          ...
          <div className="p-2.5 rounded-lg bg-zinc-900/90 border border-zinc-800 flex items-center justify-between gap-3 font-mono text-xs text-zinc-300 overflow-x-auto">
          ...
          <div className="flex items-center gap-2 font-mono text-xs shrink-0">
            {project.liveUrl && (
              <a
                ...
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-zinc-100 hover:bg-white text-zinc-950 font-medium transition-colors"
              >
                <span>Live Site</span>
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-zinc-900 hover:bg-zinc-800 text-zinc-200 border border-zinc-800 transition-colors"
            >
